...
// IsThreadSubscribed reports whether an active (non-tombstoned) subscription
// exists for the given thread. Used to decide whether a react-triggered
// auto-follow should be undone: if the user already followed the thread, leave
// it alone.
func (db *DB) IsThreadSubscribed(workspaceID, channelID, threadTS string) (bool, error) {
	const q = `SELECT EXISTS(SELECT 1 FROM thread_subscriptions WHERE workspace_id=? AND channel_id=? AND thread_ts=? AND active=1)`
	var exists int
	if err := db.conn.QueryRow(q, workspaceID, channelID, threadTS).Scan(&exists); err != nil {
