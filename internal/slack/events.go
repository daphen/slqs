...
// wsPresenceEvent represents a presence_change event. With
...
// wsTypingEvent represents a user_typing event. thread_ts is set when the user
// is typing in a thread, so the indicator can show in the thread not the channel.
...
func dispatchWebSocketEvent(data []byte, handler EventHandler) {
	...
	case "presence_change":
		...
		debuglog.WS("presence_change: user=%q users=%v presence=%q", evt.User, evt.Users, evt.Presence)
		if len(evt.Users) > 0 {
			for _, u := range evt.Users {
				handler.OnPresenceChange(u, evt.Presence)
			}
		} else if evt.User != "" {
	...
	case "user_typing":
		...
		handler.OnUserTyping(evt.Channel, evt.ThreadTS, evt.User)
